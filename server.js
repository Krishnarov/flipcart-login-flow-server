...
const MONGODB_URI =
  process.env.MONGODB_URI || "mongodb://127.0.0.1:27017/excel_login_db";
...
mongoose
  ...
  .then(async () => {
    ...
    const fixedJobs = await AutomationJob.updateMany(
      ...
      {
        status: "stopped",
        reason: "Server restarted - automation was interrupted.",
      },
    );
    if (fixedJobs.modifiedCount > 0)
      console.log(
        `Reset ${fixedJobs.modifiedCount} stuck running job(s) to stopped.`,
      );
    ...
    if (fixedEmails.modifiedCount > 0)
      console.log(
        `Reset ${fixedEmails.modifiedCount} stuck inprogress email(s) to pending.`,
      );
    server.listen(PORT, () => {
