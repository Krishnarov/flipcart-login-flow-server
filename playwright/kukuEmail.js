...
export const loginToEmail = async (page, email) => {
  try {
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
      try {
        ...
        if (isVisible) {
          ...
          console.log(
            `[Email] Mail Subject Found (attempt ${attempt}): "${subjectText}"`,
          );
        ...
        if (attempt < MAX_RETRIES) {
          console.log(
            `[Email] OTP not found yet (attempt ${attempt}/${MAX_RETRIES}). Waiting ${POLL_INTERVAL}ms...`,
          );
          await delay(POLL_INTERVAL);
        }
      } catch (pollErr) {
        console.warn(
          `[Email] Poll attempt ${attempt} error: ${pollErr.message}`,
        );
        await delay(POLL_INTERVAL);
      }
    }
    ...
    throw new Error(
      `OTP not received after ${MAX_RETRIES} polling attempts (${(MAX_RETRIES * POLL_INTERVAL) / 1000}s timeout)`,
    );
  } catch (error) {
