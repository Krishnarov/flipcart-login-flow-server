...
export const loginToFlipkartWithOTP = async (page, otp) => {
  try {
    ...
    if (count < otp.length) {
      throw new Error(
        `OTP inputs found (${count}) is less than OTP length (${otp.length})`,
      );
    }
    ...
    if (!page.url().includes("/account/login")) {
      console.log(
        "[Flipkart] Already redirected away from login page. Skipping Verify button click.",
      );
      return;
    }
    ...
    try {
      if ((await verifyBtn.count()) > 0 && (await verifyBtn.isVisible())) {
        ...
        await verifyBtn.click({ timeout: 5000 }).catch((clickErr) => {
          console.log(
            `[Flipkart] Verify button click failed/ignored (might be navigating): ${clickErr.message}`,
          );
        });
      } else {
        console.log(
          "[Flipkart] Verify button not found or not visible (might have auto-submitted).",
        );
      }
    } catch (btnErr) {
